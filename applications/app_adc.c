...
static THD_FUNCTION(adc_thread, arg) {
  ...
  for(;;) {
	...
	if (front_pad > 0.02 || brake_pad > 0.02) {
	  ...
	  brake_log_pad = pow(config.z_brakepad_linearity,brake_pad)-config.z_brakepad_offset;
	  
	  //cur_current = (front_log_pad*24.0)-(brake_log_pad*38.0);

	  // Due to the offset, can sometimes go negative!
	  if (front_log_pad <= 0.0)
	    front_log_pad = 0.0;
	  if (brake_log_pad <= 0.0)
	    brake_log_pad = 0.0;
